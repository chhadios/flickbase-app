...
router.route('/register')
    .post(async (req, res) => {
        try {
            ...
            //creating model(hashing password)
            const user = new User({
            ...
            res.cookie('access-token', token).status(200)
...
router.route('/signin')
    .post(async (req, res) => {
        try {
            ...
            if (!compare) return res.status(400).json({ message: 'password incorrect' });
            //generate token
            const token = user.generateToken();
            //response
            res.cookie('access-token', token).status(200)
    ...
    .patch(checkLoggedIn,grantAccess('readOwn','profile'),async(req,res)=>{
        try{
    ...
    .patch(checkLoggedIn,grantAccess('readOwn','profile'),async(req,res)=>{
        try{
            if(await User.emailTaken(req.body.newemail)){
                return res.status(400).json({message:'sorry email not available'})
            }
