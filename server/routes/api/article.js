...
const {sortArgHelper} = require('../../config/helpers')
//model
...
router.route('/admin/add_article')
    .post(checkLoggedIn, grantAccess('createAny', 'article'), async (req, res) => {
        try {
            ...
            res.status(400).json({message:'can not add article',error:error})
        }
    });
    ...
    .delete(checkLoggedIn, grantAccess('deleteAny', 'article'), async (req, res) => {
        try{
            ...
            if(!article) return res.status(400).json({message:'article not found'});
            res.status(200).json({message:'deleted from the data base'});
            
        }catch(error){
            res.status(400).json({message:'can not delete',error})
        }
    })
    ...
    .post(async(req,res)=>{
        try{
           let sortArgs=sortArgHelper(req.body)
